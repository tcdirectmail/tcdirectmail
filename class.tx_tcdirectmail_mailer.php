...
class tx_tcdirectmail_mailer {
	...
	public function testClickLinks () {
		...
		foreach ($urls[1] as $i => $url) {
			$tag = $urls[0][$i];
			$clickTag = str_replace($url, $this->getTClickUrl($url), $tag);
			$this->html = str_replace($tag, $clickTag, $this->html);
		}

		/* Exchange all http:// links plaintext */
		preg_match_all ('|http://[^ \n\r\)]*|i', $this->plain, $urls);
		foreach ($urls[0] as $i => $url) {
			$this->plain = str_replace($url, $this->getTClickUrl($url), $this->plain);
		}
	}

	/**
	 * Generate the URL for test clicking
	 * @param  string    $url     Url to encode
	 * @return string             Encoded url
	 */
	private function getTClickUrl($url) {
		return $this->siteUrl."index.php?eID=tclick&c=" . t3lib_div::stdAuthCode($url) . "&l=" . base64_encode(html_entity_decode($url));
	}
